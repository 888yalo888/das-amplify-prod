    ...
    const RosterTable = () => {
        const ActiveBadge = ({ status }) => {
            if (status === EntityStatus.Active) {
                return (
                    <span className="custom-badge active-badge">Active</span>
                );
            }
            return (
                <span className="custom-badge inactive-badge">Inactive</span>
            );
        };
        ...
        const RosterRows = () => {
            return sortedSite?.roster?.map((youth) => (
                <tr key={youth.id}>
                    <td className="table-data" style={{ minWidth: '200px' }}>
                        {youth?.fullName}
                    </td>
                    <td className="table-data" style={{ minWidth: '120px' }}>
                        {youth?.dateOfBirth}
                    </td>
                    {!isTabletOrMobile && (
                        <>
                            <td className="table-data">
                                {youth?.guardianFullName}
                            </td>
                            <td className="table-data">
                                {youth?.guardianPhoneNumber}
                            </td>
                        </>
                    )}
                    <td className="table-data" style={{ minWidth: '90px' }}>
                        {gradeMapper(youth?.grade)}
                    </td>
                    <td className="table-data" style={{ minWidth: '70px' }}>
                        {youth?.gender}
                    </td>
                    {!isTabletOrMobile && (
                        <td className="table-data">
                ...
                <thead>
                    <tr>
                        ...
                        >
                            Name
                            <img
                                className={sortAsc ? 'sort-asc' : ''}
                                src={sortField === 'fullName' ? SortArrow : ''}
                            />
                        </th>
                        <th
                            className="table-data"
